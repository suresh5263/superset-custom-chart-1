...
setupColors();
const sequentialSchemeRegistry = getCategoricalSchemeRegistry();
let arrayUniqueByKey = [
  ...new Map(
    sequentialSchemeRegistry.values().map((item: any) => [item.id, item]),
  ).values(),
];
        ...
        [
          {
            ...
            config: {
              ...
              choices: () =>
                arrayUniqueByKey.map((value: any) => [value.id, value.label]),
